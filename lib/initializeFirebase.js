...
import {wrapDatabase} from 'firecrypt';
...
export const rawDatabase = getDatabase(app);
export const database = wrapDatabase(rawDatabase);
const options = {
  encryption: 'none',
  compression: process.env.REVIEWABLE_COMPRESSION_ENABLED ? 'deflate' : 'none'
};
if (process.env.REVIEWABLE_ENCRYPTION_AES_KEY) {
  _.assign(options, {
    encryption: 'aes-siv', key: process.env.REVIEWABLE_ENCRYPTION_AES_KEY,
...
database.configureFireCrypt(options, specification);
